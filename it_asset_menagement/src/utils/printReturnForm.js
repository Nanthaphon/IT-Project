...
export function printReturnForm({
  ...
  /* ── Section 4: Damages table ── */
  const tierKey = (tier || 'General').toLowerCase();
  const tierCol = tierKey.includes('graphic') || tierKey.includes('dx') ? 4
                : tierKey.includes('data') ? 3
                : 2; // index in damage row [no, name, gen, data, gx, note]
  let totalFee = 0;
  const damageRows = damages.length > 0 ? damages.map((d, i) => {
    const fee = Number(d.fee) || 0;
    totalFee += fee;
    return `
      <tr>
        <td style="border:1px solid #cbd5e1;padding:5px 8px;text-align:center;font-size:10.5px;width:34px">${i+1}</td>
        <td style="border:1px solid #cbd5e1;padding:5px 8px;font-size:10.5px">${(d.name || '').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</td>
        <td style="border:1px solid #cbd5e1;padding:5px 8px;text-align:right;font-size:10.5px;font-variant-numeric:tabular-nums">${fee ? fmtTHB(fee) : '-'}</td>
      </tr>`;
  }).join('') : `
      <tr>
        <td style="border:1px solid #cbd5e1;padding:5px 8px;text-align:center;font-size:10.5px;color:#94a3b8" colspan="3">— ไม่พบความเสียหาย —</td>
      </tr>`;
    ...
    <table>
      ...
      <tbody>
        ...
        <tr style="background:#fef3c7">
          <td colspan="2" style="border:1px solid #94a3b8;padding:7px 10px;font-size:12.5px;font-weight:700;text-align:right">รวมค่าปรับทั้งหมด</td>
          <td style="border:1px solid #94a3b8;padding:7px 10px;font-size:13px;font-weight:700;text-align:right;color:#dc2626">${fmtTHB(totalFee)} บาท</td>
        </tr>
      </tbody>
    </table>
