    ...
    <div className="h-full flex flex-col bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4 border-b border-slate-100 pb-6 shrink-0">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <select value={supplyFilterDate} onChange={(e) => setSupplyFilterDate(e.target.value)} className="w-full sm:w-auto bg-white border border-slate-300 text-slate-600 px-4 py-2.5 rounded-full text-sm font-bold hover:bg-slate-50 focus:ring-2 focus:ring-[#1E487A] outline-none transition-all shadow-sm cursor-pointer">
            <option value="ทั้งหมด">วันที่: ทั้งหมด</option>
            {getUniqueDates(supplyRequests).map(d => (
              <option key={d} value={d}>{formatDateLabel(d)}</option>
            ))}
          </select>
          <select value={supplyFilterStatus} onChange={(e) => setSupplyFilterStatus(e.target.value)} className="w-full sm:w-auto bg-white border border-slate-300 text-slate-600 px-4 py-2.5 rounded-full text-sm font-bold hover:bg-slate-50 focus:ring-2 focus:ring-[#1E487A] outline-none transition-all shadow-sm cursor-pointer">
          ...
          <table className="min-w-full text-left whitespace-nowrap text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-5 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">ผู้ขอ (รหัสพนักงาน)</th>
                <th className="px-5 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">อุปกรณ์ที่ต้องการ</th>
              ...
              {currentSupplyRequests.map((req) => (
                <tr key={req.id} className="hover:bg-blue-50/40 transition-colors bg-white group">
                  ...
                  <td className="px-5 py-4 text-center">
                    <select value={req.status} onChange={(e) => handleUpdateSupplyRequestStatus(req, e.target.value)} className={`px-3 py-1.5 rounded-lg text-xs font-bold border outline-none cursor-pointer shadow-sm ${req.status === 'รอดำเนินการ' ? 'bg-amber-50 text-amber-700 border-amber-200 focus:ring-amber-500' : req.status === 'อนุมัติแล้ว' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 focus:ring-emerald-500' : 'bg-red-50 text-red-700 border-red-200 focus:ring-red-500'}`}>
                  ...
                  <td className="px-5 py-4 text-center">
                    <button onClick={() => handleDelete(req.id, 'supply_requests')} className="inline-flex items-center justify-center w-8 h-8 text-red-500 bg-white hover:bg-red-50 border border-slate-200 hover:border-red-300 rounded-lg transition-all shadow-sm">🗑️</button>
                  </td>
                </tr>
              ))}
            </tbody>
