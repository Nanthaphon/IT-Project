    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[80] transition-opacity">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden transform transition-all border border-slate-100 flex flex-col max-h-[90vh]">
        <div className="bg-[#1E487A] text-white px-6 py-5 flex justify-between items-center shrink-0">
          <h3 className="font-bold text-lg flex items-center gap-2"><span className="bg-white/20 p-1.5 rounded-lg text-sm">📥</span> {isLicense ? 'ยืนยันการรับคืนสิทธิ์' : 'ยืนยันการรับคืนอุปกรณ์'}</h3>
        ...
        <form onSubmit={handleConfirmReturn} className="p-6 md:p-8 space-y-6 overflow-y-auto flex-1">
          ...
          {isLicense && (
            <div>
              ...
              <textarea value={returnRemarks} onChange={(e) => setReturnRemarks(e.target.value)} className="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-[#1E487A] focus:border-[#1E487A] outline-none text-sm transition-all resize-none shadow-sm block" placeholder="หมายเหตุเพิ่มเติม..." rows="2"></textarea>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-4 border-t border-slate-100 shrink-0">
            ...
            <button type="submit" className={`flex-1 py-3 text-white rounded-xl font-bold transition-all shadow-lg ${isLicense || returnCondition === 'good' ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/30' : 'bg-red-600 hover:bg-red-700 shadow-red-600/30'}`}>ยืนยันการรับคืน</button>
          </div>
        </form>
      </div>
    </div>
