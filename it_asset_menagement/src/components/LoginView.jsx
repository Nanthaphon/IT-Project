         ...
         {!showAdminLogin ? (
           ...
           <div className="max-w-md mx-auto bg-slate-800 border border-slate-700 p-8 rounded-3xl shadow-2xl">
             <div className="flex items-center gap-4 mb-8">
               <button onClick={() => setShowAdminLogin(false)} className="text-slate-400 hover:text-white p-2 bg-slate-700 hover:bg-slate-600 rounded-xl transition-colors">
             ...
             <form onSubmit={handleAdminLogin} className="space-y-5">
               <div>
                 ...
                 <input type="text" value={loginForm.username} onChange={e => setLoginForm({...loginForm, username: e.target.value})} className="w-full bg-slate-900 border border-slate-700 p-3.5 rounded-xl text-white focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="ลองพิมพ์: admin" required />
               </div>
               <div>
                 <label className="block text-sm font-bold text-slate-400 mb-2">Password</label>
                 <input type="password" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} className="w-full bg-slate-900 border border-slate-700 p-3.5 rounded-xl text-white focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="ลองพิมพ์: Admin@GB" required autoComplete="new-password" />
               </div>
               <button type="submit" className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-indigo-600/30 mt-4">
                 เข้าสู่ระบบ
               </button>
             </form>
           </div>
