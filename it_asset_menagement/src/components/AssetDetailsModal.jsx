...
export default function AssetDetailsModal({
  ...
  const [tempAccDocs, setTempAccDocs] = useState([]);  // ไฟล์แนบสำหรับชิ้นย่อยที่กำลังแก้ไข
  ...
  const [newSeatNote, setNewSeatNote] = useState('');
  ...
  const [tempLicenseNote, setTempLicenseNote] = useState('');
  ...
  // 🔒 ล็อก scroll ของ background — ใช้ global observer ใน App.jsx แทนแล้ว

  if (!selectedAssetDetail) return null;

  // 🆕 Helper: เช็คสถานะหมดอายุ (ใกล้หมด/หมดแล้ว) — ใช้กับทั้ง parent + seat
  const checkLicenseExpiration = (expDate) => {
    if (!expDate) return { isExpiring: false, statusText: '', colorClass: '' };
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const d = new Date(expDate);
    const diff = Math.ceil((d - today) / (1000 * 60 * 60 * 24));
    if (diff < 0)   return { isExpiring: true, statusText: 'หมดอายุแล้ว',              colorClass: 'text-rose-600 bg-rose-50 border-rose-200' };
    if (diff <= 30) return { isExpiring: true, statusText: `เหลือ ${diff} วัน`,         colorClass: 'text-rose-600 bg-rose-50 border-rose-200' };
    if (diff <= 90) return { isExpiring: true, statusText: `เหลือ ${diff} วัน`,         colorClass: 'text-clay-600 bg-clay-100 border-clay-200' };
  ...
  /* ── คำนวณอายุการใช้งานจากวันที่ซื้อ → "X ปี Y เดือน Z วัน" ── */
  const calcAccessoryAge = (purchaseDate) => {
    if (!purchaseDate) return null;
    const start = new Date(purchaseDate);
    if (isNaN(start.getTime())) return null;
    const now = new Date();
    if (start > now) return { text: 'ยังไม่เริ่มใช้งาน', tone: 'stone', years: 0 };
    ...
    // อิงนโยบาย accessory: <1ปี = olive (ภายในอายุ Mouse/KB), 1-2ปี = ochre, >2ปี = stone (เกินแล้ว)
    const tone = years === 0 ? 'olive' : years < 2 ? 'ochre' : 'stone';
    return { text: parts.join(' '), tone, years };
  };

  /* ── อัปโหลดไฟล์แนบสำหรับชิ้นย่อย (base64 inline, จำกัด 5MB/ไฟล์) ── */
  const handleAccItemDocUpload = async (e, target) => {
    const files = Array.from(e.target.files);
    ...
    try {
      ...
      if (target === 'edit') {
        setTempAccDocs(prev => [...prev, ...newDocs]);
      } else if (target === 'new') {
        setNewItemData(prev => ({ ...prev, documents: [...(prev.documents || []), ...newDocs] }));
      }
  ...
  const handleOpenAccDoc = (doc) => {
    ...
    } else if (doc.data.startsWith('data:image/')) {
      w.document.write(`<img src="${doc.data}" style="max-width:100%;height:auto" alt="${doc.name}">`);
    } else {
  ...
  const handleAddNewPiece = async () => {
    ...
    try {
      ...
      setNewItemData({ sn: '', model: '', cost: '', purchaseDate: '', warrantyDate: '', quantity: 1, documents: [] });
    } catch (error) {
      console.error(error);
    } finally {
      setIsSavingItem(false);
    ...
    try {
      ...
      if (item.type === 'available') {
        ...
        if (Array.isArray(currentAssetDetail.availableItems)) {
          ...
          if (newItems[idx]) {
            newItems[idx] = { ...newItems[idx], sn: tempSNValue.trim(), model: tempModelValue.trim(), cost: tempCostValue.trim(), purchaseDate: tempPurchaseDateValue, warrantyDate: tempWarrantyDateValue, documents: tempAccDocs };
          }
  ...
  const handleDocumentUpload = async (e) => {
    ...
    // ไฟล์ที่ไม่ใช่รูปต้องไม่เกิน 900 KB (Firestore doc limit ~1 MB หลัง base64 = ~750 KB binary)
    const NON_IMG_LIMIT = 900 * 1024;
    const nonImgOversized = files.filter(f => !f.type.startsWith('image/') && f.size > NON_IMG_LIMIT);
    if (nonImgOversized.length > 0) {
      if (setCustomAlert) setCustomAlert({
        isOpen: true, title: 'ไฟล์ใหญ่เกินไป',
        message: `ไฟล์ที่ไม่ใช่รูปต้องไม่เกิน 900 KB (Firestore จำกัด 1 MB/เอกสาร)\n\nไฟล์ที่ใหญ่เกิน:\n${nonImgOversized.map(f => `• ${f.name} (${(f.size/1024/1024).toFixed(2)} MB)`).join('\n')}`,
        type: 'error',
      });
      e.target.value = null;
      return;
    ...
    try {
      ...
      // รูปภาพ → บีบด้วย EVIDENCE_PRESET (1000px/0.7 → ~80-120 KB)
      // ไฟล์อื่น → อ่านเป็น base64 ตามปกติ
      const newDocs = await Promise.all(files.map(async (file) => {
        if (file.type.startsWith('image/')) {
          const compressed = await compressImage(file, EVIDENCE_PRESET);
          return { name: file.name.replace(/\.(png|webp|gif|bmp|tiff?)$/i, '.jpg'), data: compressed };
        }
  ...
  /* 🆕 handlers สำหรับ photoGallery — รูปที่ใช้ประกอบใน PDF (แยกจากเอกสารแนบ) */
  const handlePhotoGalleryUpload = async (e) => {
    const files = Array.from(e.target.files || []);
    if (files.length === 0) return;
    const imageFiles = files.filter(f => f.type.startsWith('image/'));
    if (imageFiles.length === 0) {
      if (setCustomAlert) setCustomAlert({ isOpen: true, title: 'ต้องเป็นไฟล์รูปเท่านั้น', message: 'กรุณาเลือกไฟล์รูปภาพ (.jpg, .png ฯลฯ)', type: 'error' });
      e.target.value = null;
      return;
    }
    setIsSavingItem(true);
    try {
      const docRef = doc(db, selectedAssetCategory, currentAssetDetail.id);
      const compressed = await Promise.all(imageFiles.map(f => compressImage(f, EVIDENCE_PRESET)));
      const existing = Array.isArray(currentAssetDetail.photoGallery) ? currentAssetDetail.photoGallery : [];
      const combined = [...existing, ...compressed];
      await updateDoc(docRef, { photoGallery: combined });
    } catch (err) {
      console.error('Photo gallery upload failed:', err);
      if (setCustomAlert) setCustomAlert({ isOpen: true, title: 'ข้อผิดพลาด', message: 'อัปโหลดรูปไม่สำเร็จ: ' + err.message, type: 'error' });
    } finally {
      setIsSavingItem(false);
      e.target.value = null;
    }
  ...
  const handleRemovePhotoGallery = async (idx) => {
    const doRemove = async () => {
      ...
      try {
        ...
        await updateDoc(docRef, { photoGallery: updated });
      } catch (err) {
        console.error('Remove gallery photo failed:', err);
      } finally {
        setIsSavingItem(false);
      }
    };
    if (typeof showConfirm === 'function') {
      showConfirm('ยืนยันการลบรูป', 'ต้องการลบรูปนี้ออกจากคลังใช่หรือไม่?', doRemove, { confirmText: 'ยืนยันลบ', icon: 'trash' });
    } else {
  ...
  if (selectedAssetCategory === 'licenses') {
    ...
    for (let i = 0; i < availableCount; i++) {
      licenseSeats.push({
        ...
        // 🆕 per-seat fields
        seatLabel: currentAssetDetail.availableSeatLabels?.[i] || '',
        seatSupplier: currentAssetDetail.availableSeatSuppliers?.[i] || '',
        seatPurchaseDate: currentAssetDetail.availableSeatPurchaseDates?.[i] || '',
        seatExpirationDate: currentAssetDetail.availableSeatExpirationDates?.[i] || '',
        seatNote: currentAssetDetail.availableSeatNotes?.[i] || '',
    ...
    assignees.forEach((a, i) => {
      licenseSeats.push({
        ...
        seatSupplier: a.seatSupplier || '',
        seatPurchaseDate: a.seatPurchaseDate || '',
        seatExpirationDate: a.seatExpirationDate || '',
        seatNote: a.seatNote || '',
  ...
  const handleAddLicenseSeats = async () => {
    ...
    try {
      ...
      const newSuppliers = [...(currentAssetDetail.availableSeatSuppliers || [])];
      const newPurchaseDates = [...(currentAssetDetail.availableSeatPurchaseDates || [])];
      const newExpDates = [...(currentAssetDetail.availableSeatExpirationDates || [])];
      const newNotes = [...(currentAssetDetail.availableSeatNotes || [])];
      const newLabels = [...(currentAssetDetail.availableSeatLabels || [])];
      ...
      for (let i = 0; i < count; i++) {
        ...
        newSuppliers.push(newSeatSupplier.trim());
        newPurchaseDates.push(newSeatPurchaseDate);
        newExpDates.push(newSeatExpirationDate);
        newNotes.push(newSeatNote.trim());
        // 🆕 label — ถ้าไม่ใส่ → ใช้ชื่อ license หลัก เป็นค่าเริ่มต้น
        // ถ้าใส่หลายอันให้เพิ่มเลข เช่น "Sale Team #1", "Sale Team #2"
        const lbl = newSeatLabel.trim() || (currentAssetDetail.name || '');
        const baseIdx = (currentAssetDetail.availableSeatLabels?.length || 0) + i + 1;
        newLabels.push(count > 1 ? `${lbl} #${baseIdx}` : lbl);
      ...
      const updateData = {
        ...
        availableSeatSuppliers: newSuppliers,
        availableSeatPurchaseDates: newPurchaseDates,
        availableSeatExpirationDates: newExpDates,
        availableSeatNotes: newNotes,
        availableSeatLabels: newLabels,
      ...
      setNewSeatNote(''); setNewSeatLabel('');
      setNewSeatDocs([]);
  ...
  const handleSaveLicenseSeatEdit = async (seat) => {
    ...
    try {
      ...
      if (seat.type === 'available') {
        ...
        // 🆕 per-seat: supplier, purchaseDate, expirationDate, note
        const newSuppliers = pad(currentAssetDetail.availableSeatSuppliers, ''); newSuppliers[idx] = tempLicenseSupplier;
        const newPDates = pad(currentAssetDetail.availableSeatPurchaseDates, ''); newPDates[idx] = tempLicensePurchaseDate;
        const newEDates = pad(currentAssetDetail.availableSeatExpirationDates, ''); newEDates[idx] = tempLicenseExpirationDate;
        const newNotes = pad(currentAssetDetail.availableSeatNotes, ''); newNotes[idx] = tempLicenseNote;
        const newLabels = pad(currentAssetDetail.availableSeatLabels, ''); newLabels[idx] = tempLicenseLabel;
        const newDocMap = { ...(currentAssetDetail.availableSeatDocs || {}) };
        if (tempLicenseSeatDocs.length > 0) newDocMap[String(idx)] = tempLicenseSeatDocs;
        else delete newDocMap[String(idx)];
        Object.assign(updateData, {
          availableKeys: newKeys, availableKeyCodes: newKeyCodes, availableSeatCosts: newCosts,
          availableSeatSuppliers: newSuppliers, availableSeatPurchaseDates: newPDates,
          availableSeatExpirationDates: newEDates, availableSeatNotes: newNotes,
          availableSeatLabels: newLabels,
          availableSeatDocs: newDocMap,
        });
        ...
        if (idx !== -1) {
          newAssignees[idx] = {
            ...newAssignees[idx],
            productKey: tempLicenseProductKey, keyCode: tempLicenseKeyCode, seatCost: tempLicenseSeatCost,
            seatSupplier: tempLicenseSupplier,
            seatPurchaseDate: tempLicensePurchaseDate,
            seatExpirationDate: tempLicenseExpirationDate,
            seatNote: tempLicenseNote,
            seatLabel: tempLicenseLabel,
            seatDocuments: tempLicenseSeatDocs,
          };
  ...
  if (showLabelPreview) {
    ...
    return (
      <div data-modal="asset-detail" className="fixed inset-0 bg-stone-900/60 flex items-center justify-center p-4 z-[90] transition-opacity print:bg-transparent print:backdrop-blur-none" style={{ fontFamily: "Arial, sans-serif" }}>
        <style>{`
          @media print {
        ...
        <div className="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] max-w-md w-full overflow-hidden flex flex-col border border-stone-200/60 print:border-none print:shadow-none print:bg-transparent">
          <div className="bg-stone-50 border-b border-stone-200 px-5 py-4 flex justify-between items-center print:hidden">
            <h3 className="font-semibold text-stone-800">ตัวอย่างป้าย (Label Preview)</h3>
          ...
          <div className="p-10 flex justify-center items-center bg-stone-100/50 overflow-x-auto print:p-0 print:bg-transparent">
            <div id="printable-label-container" className="bg-white border-[3px] border-stone-800 p-2 flex flex-col w-[280px] text-stone-800 shrink-0 relative box-border overflow-hidden">
              <div className="flex gap-2.5">
                <div className="w-[72px] h-[72px] shrink-0 border border-stone-200 p-0.5 bg-white flex items-center justify-center self-start">
                ...
                <div className="flex flex-col text-[12px] leading-[1.3] font-medium w-full overflow-hidden justify-start pt-0.5 gap-[2px]">
              ...
              <div className="h-[34px] w-full mt-1.5 flex justify-center overflow-hidden">
          ...
          <div className="p-4 bg-white border-t border-stone-200 flex justify-end gap-3 print:hidden">
            ...
            <button onClick={() => window.print()} className="px-4 py-2 bg-[#A65F3C] text-white rounded-lg font-medium hover:bg-[#6E3B26] transition-colors">สั่งพิมพ์ (Print)</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div data-modal="asset-detail" className={asPage
      ? 'h-full'
      : 'fixed inset-0 bg-stone-950/60 flex items-center justify-center p-4 z-[80]'}>
      <div className={asPage
        ...
        <div className="px-6 py-5 border-b border-stone-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3.5 min-w-0">
            ...
            <h3 className="text-[17px] font-medium tracking-tight text-stone-900 leading-tight">
        ...
        <div className="px-5 py-3 shrink-0 bg-stone-50 border-b border-stone-100 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-[16px] font-medium text-[#A65F3C] leading-[1.5] truncate mb-1">{currentAssetDetail.name}</h2>
            <div className="flex flex-wrap items-center gap-2">
              {currentAssetDetail.type && (
                <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-stone-50 text-stone-600 border border-stone-200">
                  {currentAssetDetail.type}
                </span>
              )}
              ...
              {selectedAssetCategory === 'licenses' && (
                <>
                  ...
                  <span className="inline-flex items-center px-2.5 py-1 bg-stone-50 text-[#A65F3C] rounded-lg text-xs font-semibold border border-stone-200">
        ...
        <div className="px-6 border-b border-stone-100 flex gap-1 shrink-0 bg-white flex-wrap">
          <button
            ...
            className={`py-3.5 px-4 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px ${activeTab === 'info' ? 'border-[#A65F3C] text-[#A65F3C]' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
          >
            ข้อมูลทั่วไป
          </button>
          {selectedAssetCategory === 'assets' && (
            <button
              ...
              className={`py-3.5 px-4 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px ${activeTab === 'history' ? 'border-[#A65F3C] text-[#A65F3C]' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
            >
          ...
          {(selectedAssetCategory === 'assets' || selectedAssetCategory === 'accessories') && (
            <button
              ...
              className={`py-3.5 px-4 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px ${activeTab === 'ownership' ? 'border-[#A65F3C] text-[#A65F3C]' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
            >
              ประวัติการครอบครอง
            </button>
          )}
          {selectedAssetCategory === 'assets' && (
            <button
              ...
              className={`py-3.5 px-4 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px ${activeTab === 'licenses' ? 'border-[#A65F3C] text-[#A65F3C]' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
            >
              ซอฟต์แวร์ / License
            </button>
          )}
          <button
            ...
            className={`py-3.5 px-4 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px ${activeTab === 'docs' ? 'border-[#A65F3C] text-[#A65F3C]' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
          >
            เอกสารแนบ
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide p-6 bg-stone-50">
          ...
          {activeTab === 'info' && (
            <div className="space-y-4 animate-in fade-in duration-200">
                ...
                <div className="rounded-xl bg-stone-50/60 border border-stone-200/70 px-4 py-3.5">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-full bg-[#A65F3C] text-white font-medium text-[15px] flex items-center justify-center shrink-0">
                        {currentAssetDetail.assignedName.charAt(0)}
                      </div>
                      <div className="min-w-0">
                        <p className="text-[10.5px] text-[#A65F3C]/70 font-medium tracking-wide leading-[1.5]">ผู้ครอบครองปัจจุบัน</p>
                        <p className="text-[15px] font-medium text-stone-800 leading-[1.4] truncate">{currentAssetDetail.assignedName}</p>
                        {currentAssetDetail.department && <p className="text-[12px] text-stone-500 leading-[1.5]">{currentAssetDetail.department}</p>}
                      </div>
                    </div>
                    <div className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 bg-white rounded-lg border border-stone-200">
                      <span className="w-1.5 h-1.5 rounded-full bg-olive-500 shrink-0" />
                      <span className="text-[12px] font-semibold text-stone-600 whitespace-nowrap">กำลังถูกใช้งาน</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {selectedAssetCategory === 'assets' && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div className="bg-white rounded-xl border border-stone-200/70 px-4 py-3.5">
                    ...
                    <p className="text-[12.5px] font-medium text-stone-400">ราคาจัดซื้อ</p>
                    <p className="text-[16px] font-medium text-stone-800 leading-[1.4] mt-0.5 tabular-nums">{currentAssetDetail.cost ? `฿${Number(currentAssetDetail.cost).toLocaleString()}` : '-'}</p>
                  </div>
                  <div className="bg-white rounded-xl border border-stone-200/70 px-4 py-3.5">
                    <div className="w-9 h-9 rounded-xl bg-stone-50 text-stone-400 flex items-center justify-center mb-2.5">
              ...
              <div className="bg-white rounded-xl border border-stone-200/70 shadow-[0_1px_2px_rgba(74,43,41,0.04)] overflow-hidden">
                <div className="px-5 py-3 border-b border-stone-100 flex items-center gap-2">
                  <div className="w-1 h-4 rounded-full bg-[#A65F3C]" />
                  <h4 className="text-[13.5px] font-semibold text-stone-600">ข้อมูลจำเพาะ</h4>
                </div>
                      ...
                      {currentAssetDetail.note && (
                        <div className="col-span-2 md:col-span-4">
                          <div className="bg-clay-100/60 border border-clay-200/60 rounded-lg p-3">
                            <p className="text-[12.5px] font-medium text-clay-600/80 mb-1">หมายเหตุ / รายละเอียดเพิ่มเติม</p>
                            <p className="text-[13.5px] text-stone-700 leading-relaxed whitespace-pre-wrap">{currentAssetDetail.note}</p>
                          </div>
                        </div>
                      )}
                    </>
                      ...
                      {currentAssetDetail.remark && (
                        <div className="col-span-2 md:col-span-4">
                          <div className="bg-stone-50/60 border border-stone-200/60 rounded-lg p-3">
                            <p className="text-[12.5px] font-medium text-stone-700/80 mb-1">Remark</p>
                            <p className="text-[13.5px] text-stone-700 leading-relaxed whitespace-pre-wrap">{currentAssetDetail.remark}</p>
                          </div>
                        </div>
                      )}
                    </>
              ...
              {/* 🆕 คลังรูปภาพ (Gallery) — ใช้ประกอบใน PDF (แยกจากเอกสารแนบ) */}
              {selectedAssetCategory === 'assets' && (() => {
                  ...
                  <div className="bg-white p-4 rounded-xl border border-stone-200/70 shadow-[0_1px_2px_rgba(74,43,41,0.04)]">
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2">
                        <div className="w-1 h-4 rounded-full bg-[#A65F3C]" />
                        <h4 className="text-[13.5px] font-semibold text-stone-600">
                          รูปภาพประกอบ PDF
                          {gallery.length > 0 && <span className="ml-1.5 text-[12px] font-normal text-stone-400">({gallery.length} รูป)</span>}
                        </h4>
                      </div>
                      <label className={`cursor-pointer inline-flex items-center gap-1.5 text-[12.5px] font-semibold py-1.5 px-3 rounded-lg border transition-colors ${isSavingItem ? "border-stone-200 bg-stone-50 text-stone-400" : "border-stone-200 bg-white text-[#A65F3C] hover:bg-stone-50 hover:border-stone-200"}`}>
                        {isSavingItem ? (
                    ...
                    <p className="text-[11.5px] text-stone-400 mb-3 leading-relaxed">
                    ...
                    {gallery.length === 0 ? (
                      <div className="py-8 text-center bg-white border border-dashed border-stone-200 rounded-xl">
                        ...
                        <p className="text-[13px] font-semibold text-stone-500">ยังไม่มีรูปในคลัง</p>
                        <p className="text-[11.5px] text-stone-400 mt-0.5">กด "เพิ่มรูป" เพื่ออัปโหลด</p>
                      </div>
                    ) : (
                        ...
                        {gallery.map((src, idx) => (
                          <div key={idx} className="relative group rounded-lg overflow-hidden border border-stone-200 aspect-square bg-stone-50">
                            ...
                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                              <a href={src} target="_blank" rel="noreferrer" className="bg-white/90 text-stone-800 text-[11px] font-semibold px-2 py-1 rounded-lg hover:bg-white shadow-sm">ดู</a>
                            </div>
                            <button
              ...
              {selectedAssetCategory === 'licenses' && (
                <div className="bg-white rounded-xl border border-stone-200/70 shadow-[0_1px_2px_rgba(74,43,41,0.04)] overflow-hidden">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between px-5 py-3 border-b border-stone-100 gap-2">
                    <div className="flex items-center gap-2">
                      <div className="w-1 h-4 rounded-full bg-[#A65F3C]" />
                      <h4 className="text-[13.5px] font-semibold text-stone-600">รายการผู้ถือสิทธิ์ ({seatSearchQ ? `${visibleLicenseSeats.length}/${licenseSeats.length}` : licenseSeats.length})</h4>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      {licenseSeats.length > 0 && (
                        <label className="flex items-center gap-1.5 text-xs text-stone-600 cursor-pointer mr-2">
                          <input type="checkbox" checked={selectedLicenseSeatsForDelete.length === licenseSeats.length && licenseSeats.length > 0} onChange={handleSelectAllLicenseSeats} className="w-3.5 h-3.5 rounded border-stone-300 text-[#A65F3C] focus:ring-[#A65F3C]" />
                          เลือกทั้งหมด
                        </label>
                      )}
                      {selectedLicenseSeatsForDelete.length > 0 && (
                        <button onClick={handleDeleteSelectedLicenseSeats} disabled={isSavingItem} className="text-xs font-semibold text-rose-600 bg-white hover:bg-rose-50 px-3 py-1.5 rounded-lg border border-stone-200 hover:border-rose-300 transition-colors disabled:opacity-50">
                          ลบ ({selectedLicenseSeatsForDelete.length})
                        </button>
                      )}
                      <button onClick={() => { setIsImportingLicenseCSV(!isImportingLicenseCSV); setIsAddingNewLicenseSeat(false); }} className="text-xs font-semibold text-stone-600 bg-white hover:bg-stone-50 border border-stone-200 hover:border-stone-300 px-3 py-1.5 rounded-lg transition-colors">
                        นำเข้า CSV
                      </button>
                      <button onClick={() => { setIsAddingNewLicenseSeat(!isAddingNewLicenseSeat); setIsImportingLicenseCSV(false); }} className="text-xs font-semibold text-white bg-[#A65F3C] hover:bg-[#8E4E30] px-3 py-1.5 rounded-lg transition-colors">
                        + เพิ่มสิทธิ์
                      </button>
                    </div>
                  </div>
                    ...
                    <div className="px-5 py-2.5 border-b border-stone-100">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full bg-stone-50 border border-stone-200 rounded-lg pl-9 pr-9 py-2 text-[13px] text-stone-700 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-[#A65F3C]/20 focus:border-[#A65F3C] focus:bg-white transition"
                        />
                        {seatSearch && (
                          <button
                            type="button"
                  ...
                  {isImportingLicenseCSV && (
                    <div className="bg-stone-50 p-4 rounded-lg border border-stone-200 mb-4 animate-in fade-in">
                      <div className="flex flex-col sm:flex-row gap-4 mb-3">
                        <div className="flex-1">
                          <p className="text-xs font-semibold text-stone-700 mb-2">1. ดาวน์โหลด Template</p>
                          <button onClick={handleDownloadLicenseSeatTemplate} className="w-full py-1.5 bg-white border border-stone-200 text-stone-600 text-xs font-semibold rounded-lg hover:bg-stone-50 hover:border-stone-300 transition-colors">ดาวน์โหลด .csv</button>
                        </div>
                        <div className="flex-1">
                          <p className="text-xs font-semibold text-stone-700 mb-2">2. อัปโหลดข้อมูล</p>
                          <input type="file" accept=".csv" onChange={handleUploadLicenseSeatCSV} className="block w-full text-xs text-stone-600 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-stone-200 file:text-stone-700 cursor-pointer" />
                        </div>
                      </div>
                      <div className="text-right"><button onClick={() => setIsImportingLicenseCSV(false)} className="text-xs text-stone-500 hover:text-stone-700">ยกเลิก</button></div>
                    </div>
                  )}
                      ...
                      <section className="space-y-4">
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-[12px] font-semibold text-stone-500">ไฟล์แนบ</span>
                            <label className={`inline-flex items-center gap-1.5 cursor-pointer text-[12.5px] font-semibold py-1.5 px-3 rounded-lg border transition-colors ${isSavingItem ? 'bg-stone-100 text-stone-400 border-stone-200' : 'bg-white text-[#A65F3C] border-stone-200 hover:bg-stone-50'}`}>
                          ...
                          {newSeatDocs.length > 0 ? (
                            ...
                            <p className="text-[12.5px] text-stone-400">ยังไม่มีไฟล์แนบ</p>
                          )}
                        </div>
                      </section>
                    </ModalBody>
                  ...
                  <div className="border border-stone-200 rounded-xl overflow-hidden divide-y divide-stone-100">
                    {visibleLicenseSeats.length === 0 && seatSearchQ && (
                      <div className="py-10 text-center text-stone-400">
                        ...
                        <button type="button" onClick={() => setSeatSearch('')} className="mt-1.5 text-[12.5px] font-semibold text-[#A65F3C] hover:underline">ล้างคำค้นหา</button>
                      </div>
                    )}
                      ...
                      <div key={seat.id} className="bg-white transition-colors hover:bg-stone-50">
                        <div onClick={() => setSeatDetailModal(seat)} className="p-3 flex items-center justify-between cursor-pointer gap-2">
                          <div className="flex items-center gap-3 overflow-hidden">
                            <input type="checkbox" checked={selectedLicenseSeatsForDelete.includes(seat.id)} onChange={(e) => { e.stopPropagation(); handleSelectLicenseSeat(seat.id); }} onClick={(e) => e.stopPropagation()} className="w-3.5 h-3.5 text-[#A65F3C] rounded border-stone-300 shrink-0" />
                            ...
                            {seat.type === 'available' ? (
                              <div className="flex items-center gap-2 overflow-hidden">
                                ...
                                <div className="overflow-hidden">
                                  ...
                                  {seat.productKey && (
                                    <p className="text-[10.5px] text-stone-400 font-mono truncate flex items-center gap-1 mt-0.5" title={seat.productKey}>
                                  ...
                                  {(seat.seatExpirationDate || currentAssetDetail.expirationDate) && (
                                    <p className="text-[10.5px] text-stone-400 truncate flex items-center gap-1 mt-0.5">
                                ...
                                <span className="text-[11px] bg-olive-50 text-olive-700 border border-olive-200 px-2 py-0.5 rounded-lg font-semibold shrink-0">พร้อมใช้งาน</span>
                                {/* 🆕 Badge ใกล้หมดอายุ */}
                                {(() => {
                                  const ex = checkLicenseExpiration(seat.seatExpirationDate || currentAssetDetail.expirationDate);
                                  return ex.isExpiring ? (
                                    <span
                                      className={`inline-flex items-center gap-0.5 text-[10.5px] font-semibold px-2 py-0.5 rounded-lg border shrink-0 ${ex.colorClass}`}
                              ...
                              <div className="flex items-center gap-2.5 overflow-hidden">
                                ...
                                <div className="overflow-hidden">
                                  <p className="text-xs font-semibold text-stone-800 truncate">{seat.assignee.assignedAssetName || 'ทรัพย์สิน'}</p>
                                  {seat.assignee.empId ? (
                              ...
                              <div className="flex items-center gap-2.5 overflow-hidden">
                                <div className="w-7 h-7 rounded-full bg-stone-100 text-[#A65F3C] flex items-center justify-center font-medium text-xs border border-stone-200 shrink-0">
                                  {seat.assignee.empName?.charAt(0) || '?'}
                                </div>
                                <div className="overflow-hidden">
                                  <p className="text-xs font-semibold text-stone-800 truncate">{seat.assignee.empName}</p>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {seat.type === 'available' ? null : seat.assignee.isAssetBound ? (
                              seat.assignee.empId ? (
                                <span className="text-[10.5px] font-semibold bg-stone-50 text-stone-600 border border-stone-200 px-2 py-1 rounded-lg whitespace-nowrap">กำลังใช้งาน</span>
                              ) : (
                                <span className="text-[10.5px] font-semibold bg-clay-100 text-clay-600 border border-clay-200 px-2 py-1 rounded-lg">ติดตั้งบนเครื่อง</span>
                              )
                            ) : (
              ...
              {selectedAssetCategory === 'accessories' && (
                <div className="bg-white rounded-xl border border-stone-200/70 shadow-[0_1px_2px_rgba(74,43,41,0.04)] overflow-hidden">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between px-5 py-3 border-b border-stone-100 gap-2">
                    <div className="flex items-center gap-2">
                      <div className="w-1 h-4 rounded-full bg-[#A65F3C]" />
                      <h4 className="text-[13.5px] font-semibold text-stone-600">รายการชิ้นย่อย ({accSearchQ ? `${visibleIndividualItems.length}/${individualItems.length}` : individualItems.length})</h4>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      {individualItems.length > 0 && (
                        <label className="flex items-center gap-1.5 text-xs text-stone-600 cursor-pointer mr-2">
                          <input type="checkbox" checked={selectedItemsForDelete.length === individualItems.length && individualItems.length > 0} onChange={handleSelectAllItems} className="w-3.5 h-3.5 rounded border-stone-300 text-[#A65F3C] focus:ring-[#A65F3C]" />
                          เลือกทั้งหมด
                        </label>
                      )}
                      {selectedItemsForDelete.length > 0 && (
                        <button onClick={handleDeleteSelectedItems} disabled={isSavingItem} className="text-xs font-semibold text-rose-600 bg-white hover:bg-rose-50 px-3 py-1.5 rounded-lg border border-stone-200 hover:border-rose-300 transition-colors disabled:opacity-50">
                          ลบ ({selectedItemsForDelete.length})
                        </button>
                      )}
                      <button onClick={() => { setIsImportingCSV(!isImportingCSV); setIsAddingNew(false); }} className="text-xs font-semibold text-stone-600 bg-white hover:bg-stone-50 border border-stone-200 hover:border-stone-300 px-3 py-1.5 rounded-lg transition-colors">
                        นำเข้า CSV
                      </button>
                      <button onClick={() => { setIsAddingNew(!isAddingNew); setIsImportingCSV(false); }} className="text-xs font-semibold text-white bg-[#A65F3C] hover:bg-[#8E4E30] px-3 py-1.5 rounded-lg transition-colors">
                        + เพิ่มชิ้นใหม่
                      </button>
                    </div>
                  </div>
                  ...
                  {isImportingCSV && (
                    <div className="bg-stone-50 p-4 rounded-lg border border-stone-200 mb-4 animate-in fade-in">
                      <div className="flex flex-col sm:flex-row gap-4 mb-3">
                        <div className="flex-1">
                          <p className="text-xs font-semibold text-stone-700 mb-2">1. ดาวน์โหลด Template</p>
                          <button onClick={handleDownloadPieceTemplate} className="w-full py-1.5 bg-white border border-stone-200 text-stone-600 text-xs font-semibold rounded-lg hover:bg-stone-50 hover:border-stone-300 transition-colors">
                            ดาวน์โหลด .csv
                          </button>
                        </div>
                        <div className="flex-1">
                          <p className="text-xs font-semibold text-stone-700 mb-2">2. อัปโหลดข้อมูล</p>
                          <input type="file" accept=".csv" onChange={handleUploadPieceCSV} className="block w-full text-xs text-stone-600 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-stone-200 file:text-stone-700 cursor-pointer" />
                        </div>
                      </div>
                      <div className="text-right"><button onClick={() => setIsImportingCSV(false)} className="text-xs text-stone-500 hover:text-stone-700">ยกเลิก</button></div>
                    </div>
                  )}
                  ...
                  <Modal open={isAddingNew} onClose={() => setIsAddingNew(false)} size="lg">
                    ...
                    <ModalBody className="space-y-7">
                      ...
                      <section className="space-y-4">
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-[12px] font-semibold text-stone-500">ใบเสร็จ / รูป / ใบรับประกัน</span>
                            <label className={`inline-flex items-center gap-1.5 cursor-pointer text-[12.5px] font-semibold py-1.5 px-3 rounded-lg border transition-colors ${isSavingItem ? 'bg-stone-100 text-stone-400 border-stone-200' : 'bg-white text-[#A65F3C] border-stone-200 hover:bg-stone-50'}`}>
                              <Paperclip className="h-3.5 w-3.5" strokeWidth={2} /> แนบไฟล์
                  ...
                  <div className="border border-stone-200 rounded-xl overflow-hidden divide-y divide-stone-100">
                    {visibleIndividualItems.length === 0 && accSearchQ && (
                      <div className="py-10 text-center text-stone-400">
                        ...
                        <button type="button" onClick={() => setAccItemSearch('')} className="mt-1.5 text-[12.5px] font-semibold text-[#A65F3C] hover:underline">ล้างคำค้นหา</button>
                      </div>
                    )}
                    {visibleIndividualItems.map((item) => (
                      <div key={item.id} className="bg-white transition-colors hover:bg-stone-50">
                        <div onClick={() => setAccItemDetailModal(item)} className="p-3 flex items-center justify-between cursor-pointer gap-2">
                          <div className="flex items-center gap-3 overflow-hidden">
                            <input type="checkbox" checked={selectedItemsForDelete.includes(item.id)} onChange={(e) => { e.stopPropagation(); handleSelectItem(item.id); }} onClick={(e) => e.stopPropagation()} className="w-3.5 h-3.5 text-[#A65F3C] rounded border-stone-300 shrink-0" />
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-lg border shrink-0 ${item.type === 'available' ? 'bg-olive-50 text-olive-700 border-olive-200' : item.type === 'assigned' ? 'bg-stone-50 text-stone-700 border-stone-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>{item.status}</span>
                              </div>
                              {(item.sn || item.model) && (
                                <p className="text-[10.5px] text-stone-400 truncate mt-0.5 flex items-center gap-2">
                    ...
                    {individualItems.length === 0 && (
                      <div className="py-8 text-center text-[13px] text-stone-400 bg-stone-50">ไม่มีข้อมูลชิ้นย่อย</div>
                    )}
                  </div>
                  </div>
                </div>
              ...
              {!isAddingHistory && !editingHistoryId ? (
                <>
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <div className="w-1 h-4 rounded-full bg-[#A65F3C]" />
                      <h4 className="text-[13px] font-semibold text-stone-600">ประวัติการจัดซื้อ ({purchaseHistory.length} ครั้ง)</h4>
                    </div>
                    <button
                      onClick={() => {
                  ...
                  {purchaseHistory.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {purchaseHistory.map((hist, index) => (
                        <div key={hist.id} className={`bg-white rounded-xl px-5 py-4 transition-colors border ${index === 0 ? 'border-[#A65F3C]/40' : 'border-stone-200 hover:border-stone-300'}`}>
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2.5 min-w-0">
                              {index === 0 && (
                                <span className="bg-stone-50 text-[#A65F3C] px-2 py-0.5 rounded-lg text-[10.5px] font-semibold border border-stone-200 shrink-0">ล่าสุด</span>
                              )}
                            ...
                            <div className="text-[16px] font-medium text-[#A65F3C] shrink-0">
                              {hist.cost ? `฿${Number(hist.cost).toLocaleString()}` : '-'}
                            </div>
                            <div className="flex-1 min-w-0 hidden sm:block">
                              <p className="text-[13px] text-stone-600 truncate"><span className="text-stone-400">Vendor:</span> {hist.vendor || '-'}</p>
                          ...
                          {(hist.note || (hist.documents && hist.documents.length > 0)) && (
                            <div className="mt-3 pt-3 border-t border-stone-100 flex flex-wrap gap-2 items-start">
                              {hist.note && <p className="text-[13px] text-stone-500 flex-1">{hist.note}</p>}
                              {hist.documents?.map((docItem, idx) => (
                                <button
                                  ...
                                  className="flex items-center gap-1 text-[12px] bg-white text-stone-500 border border-stone-200 py-1 px-2.5 rounded-lg hover:bg-stone-50 hover:text-[#A65F3C] hover:border-stone-200 transition-colors max-w-[160px] truncate disabled:opacity-60"
                                >
                ...
                <div className="bg-white border border-stone-200/70 p-6 rounded-xl shadow-[0_1px_2px_rgba(74,43,41,0.04)]">
                  <div className="flex items-center gap-2 mb-5 pb-4 border-b border-stone-100">
                    <div className="w-1 h-5 rounded-full bg-[#A65F3C]" />
                  ...
                  <form onSubmit={handleSaveHistory} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <DateField value={historyForm.purchaseDate} onChange={(v) => setHistoryForm({...historyForm, purchaseDate: v})} inputClassName="w-full bg-white border border-stone-200 rounded-lg px-3.5 py-2.5 pr-9 text-sm text-stone-700 focus:outline-none focus:ring-2 focus:ring-[#A65F3C]/20 focus:border-[#A65F3C] transition-colors" />
                      </div>
                      <div>
                        <label className="block text-[13.5px] font-medium text-stone-600 mb-1.5">ราคา (บาท)</label>
                        <input type="number" step="any" value={historyForm.cost} onChange={(e) => setHistoryForm({...historyForm, cost: e.target.value})} className="w-full bg-white border border-stone-200 rounded-lg px-3.5 py-2.5 text-sm text-stone-700 focus:outline-none focus:ring-2 focus:ring-[#A65F3C]/20 focus:border-[#A65F3C] transition-colors" placeholder="ยอดรวมหรือต่อชิ้น" />
                      </div>
                      <div>
                        <label className="block text-[13.5px] font-medium text-stone-600 mb-1.5">ผู้จัดจำหน่าย (Vendor)</label>
                        <input type="text" value={historyForm.vendor} onChange={(e) => setHistoryForm({...historyForm, vendor: e.target.value})} className="w-full bg-white border border-stone-200 rounded-lg px-3.5 py-2.5 text-sm text-stone-700 focus:outline-none focus:ring-2 focus:ring-[#A65F3C]/20 focus:border-[#A65F3C] transition-colors" placeholder="ชื่อร้าน/บริษัท" />
                      </div>
                      <div>
                        <label className="block text-[13.5px] font-medium text-stone-600 mb-1.5">ยี่ห้อ/รุ่น (Model)</label>
                        <input type="text" value={historyForm.model} onChange={(e) => setHistoryForm({...historyForm, model: e.target.value})} className="w-full bg-white border border-stone-200 rounded-lg px-3.5 py-2.5 text-sm text-stone-700 focus:outline-none focus:ring-2 focus:ring-[#A65F3C]/20 focus:border-[#A65F3C] transition-colors" />
                      </div>
                      <div className="md:col-span-2">
                        <label className="block text-[13.5px] font-medium text-stone-600 mb-1.5">รายละเอียด / หมายเหตุ</label>
                        <textarea value={historyForm.note} onChange={(e) => setHistoryForm({...historyForm, note: e.target.value})} className="w-full bg-white border border-stone-200 rounded-lg px-3.5 py-2.5 text-sm text-stone-700 focus:outline-none focus:ring-2 focus:ring-[#A65F3C]/20 focus:border-[#A65F3C] transition-colors resize-none" rows="2" placeholder="เช่น จัดซื้อทดแทนเครื่องเดิม..."></textarea>
                      </div>
                    </div>

                    <div className="pt-3 border-t border-stone-100">
                      <label className="block text-[13.5px] font-medium text-stone-600 mb-3">เอกสารแนบการจัดซื้อ</label>
                      ...
                      {historyForm.documents && historyForm.documents.length > 0 && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                          {historyForm.documents.map((docItem, idx) => (
                            <div key={idx} className="flex items-center justify-between bg-stone-50 border border-stone-200 p-2.5 rounded-lg">
                              <button
                                type="button"
                                onClick={() => handleOpenPurchaseDoc(docItem)}
                                className="inline-flex items-center gap-1.5 text-[13px] font-medium text-[#A65F3C] hover:underline max-w-[180px] text-left"
                                title={docItem.name}
                              >
                                {openingPhDocId === docItem.id
                      ...
                      <label className={`cursor-pointer inline-flex items-center gap-1.5 text-[13px] font-semibold py-2 px-3.5 rounded-lg border transition-colors ${isSavingItem ? 'border-stone-200 bg-stone-50 text-stone-400' : 'border-stone-200 bg-white text-[#A65F3C] hover:bg-stone-50 hover:border-stone-200'}`}>
                    ...
                    <div className="flex gap-3 pt-4 border-t border-stone-100">
                      <button type="button" onClick={() => { setIsAddingHistory(false); setEditingHistoryId(null); setPendingPurchaseDocs([]); }} className="flex-1 py-2.5 bg-stone-100 text-stone-700 rounded-xl hover:bg-stone-200 text-sm font-semibold transition-colors">ยกเลิก</button>
                      <button type="submit" disabled={isSavingItem} className="flex-1 py-2.5 bg-[#A65F3C] text-white rounded-xl hover:bg-[#8E4E30] text-sm font-semibold transition-colors shadow-sm disabled:opacity-50">บันทึกประวัติ</button>
                    </div>
                  </form>
                </div>
              )}
            ...
            <div className="space-y-3 animate-in fade-in duration-200">
              <div className="bg-white p-4 rounded-xl border border-stone-200/70 shadow-[0_1px_2px_rgba(74,43,41,0.04)]">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-1 h-4 rounded-full bg-[#A65F3C]" />
                  <h4 className="text-[13px] font-semibold text-stone-600">จัดการเอกสารแนบ</h4>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    return docs.length > 0 ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        {docs.map((docItem, idx) => {
                          // 🆕 รูปภาพ → แสดง thumbnail เล็กๆ (48px) ให้เห็นว่าแนบรูปอะไร
                          const isImg = String(docItem.data || '').startsWith('data:image/');
                          return (
                          <div key={idx} className="flex items-center justify-between bg-white border border-stone-200 p-3 rounded-lg overflow-hidden">
                            <a href={docItem.data} download={docItem.name} target={isImg ? '_blank' : undefined} rel={isImg ? 'noreferrer' : undefined} className="flex items-center gap-3 text-[13.5px] text-[#A65F3C] font-medium hover:underline truncate mr-3 min-w-0">
                              {isImg ? (
                                <img
                                  src={docItem.data}
                                  alt={docItem.name}
                                ...
                                <div className="w-12 h-12 rounded-lg bg-stone-50 border border-stone-200 flex items-center justify-center shrink-0">
                                  <svg className="w-5 h-5 text-[#A65F3C]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                </div>
                              )}
                              <span className="truncate">{docItem.name}</span>
                            </a>
                  ...
                  <div className="mt-3 flex justify-center">
                    <label className={`cursor-pointer inline-flex items-center gap-1.5 text-[13.5px] font-semibold py-2.5 px-5 rounded-lg border transition-colors ${isSavingItem ? 'border-stone-200 bg-stone-50 text-stone-400' : 'border-stone-200 bg-white text-[#A65F3C] hover:bg-stone-50 hover:border-stone-200'}`}>
                      {isSavingItem ? (
        ...
        <div className="px-5 py-3 bg-white flex flex-wrap justify-end items-center gap-2 border-t border-stone-100 shrink-0 rounded-b-2xl">
          {selectedAssetCategory === 'assets' && (
            (!currentAssetDetail.status || currentAssetDetail.status === 'พร้อมใช้งาน') ? (
              <button onClick={() => { setCheckoutModal({ isOpen: true, assetId: currentAssetDetail.id, collectionName: selectedAssetCategory }); closeAll(); }} className="w-full sm:w-auto px-5 py-2.5 bg-[#A65F3C] text-white rounded-lg hover:bg-[#8E4E30] text-[14px] font-semibold transition-colors sm:mr-auto">เบิกจ่าย</button>
            ) : currentAssetDetail.status === 'ถูกใช้งาน' ? (
              <button onClick={() => { setReturnModal({ isOpen: true, assetId: currentAssetDetail.id, collectionName: 'assets', empId: currentAssetDetail.assignedTo, empName: currentAssetDetail.assignedName, assetName: currentAssetDetail.name }); closeAll(); }} className="w-full sm:w-auto px-5 py-2.5 bg-[#A65F3C] text-white rounded-lg hover:bg-[#8E4E30] text-[14px] font-semibold transition-colors sm:mr-auto">รับคืน</button>
            ) : null
          )}
          ...
          {selectedAssetCategory === 'licenses' && (() => {
            ...
            return availCount > 0 ? (
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-[#A65F3C] text-white rounded-lg hover:bg-[#8E4E30] text-[14px] font-semibold transition-colors sm:mr-auto"
              >
          ...
          <button onClick={closeAll} className="w-full sm:w-auto px-5 py-2.5 bg-[#A65F3C] text-white rounded-xl hover:bg-[#8E4E30] text-sm font-medium transition-colors">ปิด</button>
        </div>
      </div>
      ...
      {seatDetailModal && (
        <SeatDetailModal
          seat={seatDetailModal}
          license={currentAssetDetail}
          checkLicenseExpiration={checkLicenseExpiration}
          calculateAge={calculateAge}
          onClose={() => setSeatDetailModal(null)}
          onEdit={() => {
            // เปิด edit mode + setTempLicense*
            setEditingLicenseSeatId(seatDetailModal.id);
            setTempLicenseProductKey(seatDetailModal.productKey);
            setTempLicenseKeyCode(seatDetailModal.keyCode);
            setTempLicenseSeatCost(seatDetailModal.seatCost);
            setTempLicenseSeatDocs(seatDetailModal.documents || []);
            setTempLicenseSupplier(seatDetailModal.seatSupplier || currentAssetDetail.supplier || '');
            setTempLicensePurchaseDate(seatDetailModal.seatPurchaseDate || currentAssetDetail.purchaseDate || '');
            setTempLicenseExpirationDate(seatDetailModal.seatExpirationDate || currentAssetDetail.expirationDate || '');
            setTempLicenseNote(seatDetailModal.seatNote || '');
            setTempLicenseLabel(seatDetailModal.seatLabel || '');
          }}
          isEditing={editingLicenseSeatId === seatDetailModal.id}
          tempLicenseLabel={tempLicenseLabel}
          setTempLicenseLabel={setTempLicenseLabel}
          tempLicenseProductKey={tempLicenseProductKey}
          setTempLicenseProductKey={setTempLicenseProductKey}
          tempLicenseKeyCode={tempLicenseKeyCode}
          setTempLicenseKeyCode={setTempLicenseKeyCode}
          tempLicenseSupplier={tempLicenseSupplier}
          setTempLicenseSupplier={setTempLicenseSupplier}
          tempLicenseSeatCost={tempLicenseSeatCost}
          setTempLicenseSeatCost={setTempLicenseSeatCost}
          tempLicensePurchaseDate={tempLicensePurchaseDate}
          setTempLicensePurchaseDate={setTempLicensePurchaseDate}
          tempLicenseExpirationDate={tempLicenseExpirationDate}
          setTempLicenseExpirationDate={setTempLicenseExpirationDate}
          tempLicenseNote={tempLicenseNote}
          setTempLicenseNote={setTempLicenseNote}
          tempLicenseSeatDocs={tempLicenseSeatDocs}
          setTempLicenseSeatDocs={setTempLicenseSeatDocs}
          handleLicenseSeatDocUpload={handleLicenseSeatDocUpload}
          isSavingItem={isSavingItem}
          onCancelEdit={() => setEditingLicenseSeatId(null)}
          onSaveEdit={async () => {
            await handleSaveLicenseSeatEdit(seatDetailModal);
            // refresh modal data
            const updated = licenseSeats.find(s => s.id === seatDetailModal.id);
            if (updated) setSeatDetailModal(updated);
          }}
        />
      )}
...
function SeatDetailModal({
  ...
  tempLicenseKeyCode, setTempLicenseKeyCode,
  tempLicenseSupplier, setTempLicenseSupplier,
  tempLicenseSeatCost, setTempLicenseSeatCost,
  tempLicensePurchaseDate, setTempLicensePurchaseDate,
  tempLicenseExpirationDate, setTempLicenseExpirationDate,
  tempLicenseNote, setTempLicenseNote,
  tempLicenseSeatDocs, setTempLicenseSeatDocs,
  handleLicenseSeatDocUpload, isSavingItem, onCancelEdit, onSaveEdit,
}) {
  ...
  const expCheck = (seat.seatExpirationDate || license.expirationDate)
    ...
    : null;
  const age = calculateAge(seat.seatPurchaseDate || license.purchaseDate);
  ...
  return (
    <div className="fixed inset-0 bg-stone-950/60 z-[95] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl border border-stone-200/70 shadow-[0_1px_2px_rgba(74,43,41,0.04),0_24px_60px_-24px_rgba(74,43,41,0.28)] max-w-3xl w-full max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-stone-100">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3.5 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-[12.5px] font-medium text-stone-400 mb-0.5">
                  รายการย่อย · {license.name}
                </p>
                <h3 className="text-[20px] font-medium text-stone-900 leading-tight break-words">
            ...
            <div className="px-6 sm:px-7 divide-y divide-stone-100">
              ...
              {seat.type === 'assigned' && seat.assignee && (
                <Section icon={seat.assignee.isAssetBound ? Laptop : User} title="ผู้ถือครอง / การใช้งาน">
                  <div className="flex items-start gap-3.5 rounded-xl bg-stone-50 border border-stone-100 p-4">
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-medium text-[15px] shrink-0 ${seat.assignee.isAssetBound ? 'bg-clay-100 text-clay-600' : 'bg-stone-100 text-[#A65F3C]'}`}>
                      {seat.assignee.isAssetBound
                        ? <Laptop className="h-5 w-5" strokeWidth={2} />
                        : (seat.assignee.empName?.charAt(0) || '?')}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-[15px] font-medium text-stone-800 break-words">
                        {seat.assignee.empName
                      ...
                      {seat.assignee.remarks && (
                        <p className="mt-3 pt-3 border-t border-stone-200/70 text-[13px] text-stone-600 leading-relaxed">
                          <span className="text-stone-400">หมายเหตุการมอบ: </span>{seat.assignee.remarks}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              {seat.seatNote && (
                <Section icon={FileText} title="หมายเหตุ">
                  <p className="text-[13.5px] text-stone-700 whitespace-pre-wrap leading-relaxed">{seat.seatNote}</p>
                </Section>
              )}
                ...
                {seat.documents?.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {seat.documents.map((d, i) => (
                      <a key={i} href={d.data} download={d.name} className="inline-flex items-center gap-1.5 bg-stone-50 border border-stone-200 px-3 py-2 rounded-lg text-[12.5px] font-medium text-[#A65F3C] hover:bg-stone-50 hover:border-stone-200 transition-colors">
                        <Paperclip className="h-3.5 w-3.5 shrink-0" strokeWidth={2} />
                  ...
                  <p className="text-[13px] text-stone-300">— ไม่มีไฟล์แนบ</p>
                )}
              </Section>
            </div>
          )}
          ...
          {isEditing ? (
            <>
              ...
              <button
                onClick={onSaveEdit}
                disabled={isSavingItem}
                className="px-5 py-2 rounded-lg text-[13px] font-semibold text-white bg-[#A65F3C] hover:bg-[#8E4E30] shadow-sm disabled:opacity-50"
              >
            ...
            <>
              ...
              <button
                onClick={onEdit}
                className="inline-flex items-center gap-1.5 px-5 py-2 rounded-lg text-[13px] font-semibold text-white bg-[#A65F3C] hover:bg-[#8E4E30] shadow-sm"
              >
                ...
                <div>
                  ...
                  {tempAccDocs.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {tempAccDocs.map((d, i) => (
                        <div key={i} className="inline-flex items-center gap-1.5 bg-stone-50 border border-stone-200 px-3 py-1.5 rounded-lg text-[12px]">
                          <button type="button" onClick={() => handleOpenAccDoc(d)} className="text-stone-600 hover:text-[#A65F3C] truncate max-w-[160px]">{d.name}</button>
            ...
            <div className="px-6 sm:px-7 divide-y divide-stone-100">
              {item.type === 'assigned' && item.assignee && (
                <Section icon={User} title="ผู้ถือครอง / การใช้งาน">
                  <div className="flex items-start gap-3.5 rounded-xl bg-stone-50 border border-stone-100 p-4">
                    <div className="w-11 h-11 rounded-xl flex items-center justify-center font-medium text-[15px] shrink-0 bg-stone-100 text-[#A65F3C]">
                      {item.assignee.empName?.charAt(0) || '?'}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-[15px] font-medium text-stone-800 break-words">{item.assignee.empName || '-'}</p>
                      <div className="mt-1.5 grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-1">
              ...
              <Section icon={Paperclip} title={`ไฟล์แนบ${docs.length ? ` (${docs.length})` : ''}`}>
                {docs.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {docs.map((d, i) => (
                      <button key={i} type="button" onClick={() => handleOpenAccDoc(d)} className="inline-flex items-center gap-1.5 bg-stone-50 border border-stone-200 px-3 py-2 rounded-lg text-[12.5px] font-medium text-[#A65F3C] hover:bg-stone-50 hover:border-stone-200 transition-colors">
                        <Paperclip className="h-3.5 w-3.5 shrink-0" strokeWidth={2} />
                        <span className="truncate max-w-[180px]">{d.name}</span>
                      </button>
                    ))}
        ...
        <div className="px-6 py-4 border-t border-stone-100 bg-white flex justify-end gap-2.5">
          {isEditing ? (
            <>
              ...
              <button onClick={onSaveEdit} disabled={isSavingItem} className="px-5 py-2 rounded-lg text-[13px] font-semibold text-white bg-[#A65F3C] hover:bg-[#8E4E30] shadow-sm disabled:opacity-50">{isSavingItem ? 'กำลังบันทึก...' : '✓ บันทึก'}</button>
            </>
          ) : (
            <>
              ...
              <button onClick={onEdit} className="inline-flex items-center gap-1.5 px-5 py-2 rounded-lg text-[13px] font-semibold text-white bg-[#A65F3C] hover:bg-[#8E4E30] shadow-sm">
...
function KV({ icon: Icon, label, value }) {
  return (
    <div className="flex items-center gap-1.5 min-w-0">
      {Icon && <Icon className="h-3.5 w-3.5 text-stone-300 shrink-0" strokeWidth={2} />}
      <span className="text-[12.5px] text-stone-400 shrink-0">{label}</span>
      <span className="text-sm font-medium text-stone-800 truncate">{value}</span>
    </div>
  );
}
...
function KeyRow({ label, value }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-[12.5px] text-stone-400 w-28 sm:w-32 shrink-0">{label}</span>
      {has ? (
        <>
          <span className="flex-1 font-mono text-[13.5px] font-medium text-stone-800 break-all select-all">{value}</span>
          <button type="button" onClick={copy} title="คัดลอก" className="shrink-0 text-stone-400 hover:text-[#A65F3C] transition-colors">
            {copied ? <Check className="h-4 w-4 text-olive-500" strokeWidth={2.4} /> : <Copy className="h-4 w-4" strokeWidth={2} />}
          </button>
        </>
      ) : (
        <span className="flex-1 text-[13px] text-stone-300">-</span>
      )}
    </div>
  );
}
...
function DItem({ label, value, badge, span }) {
  ...
  return (
    <div className={`min-w-0 ${spanCls}`}>
      <dt className="text-[12.5px] text-stone-400 mb-1">{label}</dt>
