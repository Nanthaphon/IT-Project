          ...
          <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
            <td className="px-5 py-4">
              <button 
                ...
                className="text-left font-bold text-slate-800 group-hover:text-teal-600"
              >
                {item.name}
              </button>
            </td>
            ...
            <td className="px-5 py-4">
              {!item.status || item.status === 'พร้อมใช้งาน' ? (
                <div className="bg-teal-50 text-teal-700 px-3 py-1.5 rounded-lg text-xs font-bold inline-flex border border-teal-200 shadow-sm">
                  <span className="w-2 h-2 rounded-full bg-teal-500 mr-2 animate-pulse"></span> พร้อมใช้งาน
                </div>
              ) : (
            ...
            <td className="px-5 py-4 text-center space-x-2">
              {!item.status || item.status === 'พร้อมใช้งาน' ? (
                ...
                <button 
                  ...
                  className="px-3 py-1.5 bg-white text-emerald-600 border border-slate-300 hover:bg-emerald-50 hover:border-emerald-300 rounded-lg font-semibold transition-all text-xs shadow-sm" 
                  title="รับคืน"
                >
                  รับคืน
                </button>
